...
def quicksort(l):
	'''
	1. select some element pivot
	2. move all elements < (resp. >) than pivot to left (resp. right) of pivot
	3. call quicksort on the left and right unsorted arrays
	Base case is list with 0 elements; already sorted
	'''
	if len(l) == 0:
		return l
	left = []
	...
	pivots = []
	# print(len(l), l)
	pivot = l[len(l)//2-1] # select pivot to be in the middle
	for x in l:
		# put each element either to the left or right of pivot element
		if x == pivot:
			pivots.append(x)
		elif x < pivot:
			left.append(x)
